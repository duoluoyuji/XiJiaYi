...
public class SteamDepotService : ISteamDepotService
{
    ...
    private string _currentSource = "ShikiLua";
    ...
    /// <summary>内置高速密钥源；未配置托管时由内置离线数据兜底。</summary>
    ...
    private static readonly string[] SourceNames = ["ShikiLua", "DepotKey2"];
    ...
    /// <summary>随程序分发的内置密钥库目录（Data/ShikiLua），离线可用。</summary>
    ...
    private bool TryCopyBundledShikiLua(string depotPath, string tokenPath)
    {
        try
        {
            ...
            LogService.Info("入库", "内置离线密钥库已就绪");
            return true;
        }
        catch (Exception ex)
        {
            LogService.Warn("入库", $"复制内置密钥库失败: {ex.Message}");
            return false;
        }
    }
    ...
    private async Task<bool> ResolveKeyUrlsAsync(CancellationToken ct = default)
    {
        ...
        {
            _resolvedUrls["ShikiLua"] = (ShikiLuaDepotKeysUrl, ShikiLuaTokenKeysUrl);
            return true;
        }

        if (_currentSource == "DepotKey2")
        {
            _resolvedUrls["DepotKey2"] = (Source2DepotKeysUrl, Source2TokenKeysUrl);
            return true;
        }
    ...
    public async Task<KeyFileUpdateResult> UpdateKeyFilesAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (_currentSource == "ShikiLua" && TryCopyBundledShikiLua(depotPath, tokenPath))
            {
                ...
                catch (Exception ex) { LogService.Warn("入库", $"解析内置密钥库失败: {ex.Message}"); }
    ...
    public async Task<string?> GenerateLuaAsync(int appId, string? keyFolderPath = null, CancellationToken ct = default)
    {
        try
        {
            ...
            sb.AppendLine($"-- update time: {DateTime.Now:yyyy-MM-dd}");
            var gameName = !string.IsNullOrEmpty(queryResult.AppName) ? queryResult.AppName : LuaFileManager.TryGetLocalGameName(appId);
            if (!string.IsNullOrEmpty(gameName))
            {
                sb.AppendLine($"-- app_name: {gameName}");
            }
            sb.AppendLine();
            var matchedItems = 0;
            ...
            foreach (var depot in queryResult.GameDepots)
            {
                ...
                // 无 key 的子 depot 不生成 addappid，避免 Steam 报错 "Missing decryption key" (内容仍处于加密状态)
            }
            ...
            var scriptPath = Path.Combine(luaFolder, $"{appId}.ks");
            var oldLua = Path.Combine(luaFolder, $"{appId}.lua");
            if (File.Exists(oldLua)) try { File.Delete(oldLua); } catch { }
