...
namespace SteamLuaManager.ViewModels;
	...
	[ObservableProperty]
	private GameInfo? _selectedGameForOnlineFix;

	[RelayCommand]
	private async Task LaunchOnlineFixAsync()
	{
		if (SelectedGameForOnlineFix == null)
		{
			await ShowModernDialogAsync("提示", "请先选择一个游戏");
			return;
		}
		await LaunchGameOnlineFixAsync(SelectedGameForOnlineFix);
	}
	...
	/// <summary>以 Online Fix 模式启动游戏（steam.exe -applaunch &lt;AppID&gt; -onlinefix），由 OpenSteamTool 内核接管联机。</summary>
	public async Task LaunchGameOnlineFixAsync(GameInfo game)
	{
		if (game.IsDisabled)
		{
		...
		{
			await ShowModernDialogAsync("提示", $"未找到 steam.exe：{steamExe}");
			return;
		}
		...
		try
		{
			var psi = new ProcessStartInfo(steamExe)
			{
